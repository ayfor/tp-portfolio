			...
			<div className="mx-auto max-w-full px-2 sm:px-6 lg:px-8">
				...
				<Disclosure.Panel className="xl:hidden">
					<div className="space-y-1 px-2 pt-2 pb-3">
						{navigation.map((item) => (
							<Disclosure.Button key={item.name} as="a" href={item.href} className="block w-100 sm:w-5/12 px-10 py-2 rounded-lg text-center font-medium bg-white drop-shadow-md hover:bg-primary hover:text-white transition duration-150 ease-in-out" aria-current={item.current ? "page" : undefined}>
								{item.name}
							</Disclosure.Button>
						))}
					</div>
