				...
				<div className="mx-auto max-w-full px-2 sm:px-6 lg:px-8">
						...
						<div className="flex flex-1 items-center justify-center">
							<div className="hidden w-full flex-row justify-end items-center sm:mx-6 xl:flex">
								<div className="w-1/4 flex justify-between space-x-4">
									{navigation.map((item) => (
										<button key={item.name} className="text-black font-medium bg-white drop-shadow-xl px-4 py-2 rounded-lg w-1/4 hover:bg-black hover:text-white transition duration-150 ease-in-out" aria-current={item.current ? "page" : undefined}>
											{item.name}
										</button>
									))}
								</div>
					...
					<Disclosure.Panel className="xl:hidden">
						<div className="space-y-1 px-2 pt-2 pb-3">
							{navigation.map((item) => (
								<Disclosure.Button key={item.name} as="a" href={item.href} className="block w-100 sm:w-5/12 px-10 py-2 rounded-lg text-center font-medium bg-white drop-shadow-md hover:bg-black hover:text-white transition duration-150 ease-in-out" aria-current={item.current ? "page" : undefined}>
									{item.name}
								</Disclosure.Button>
							))}
						</div>
