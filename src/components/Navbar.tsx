				...
				<>
					<div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
						<div className="relative flex h-16 items-center justify-between">
							...
							<div className="flex flex-1 items-center justify-center">
								<div className="hidden w-4/5 flex-row justify-end items-center sm:ml-6 sm:flex">
									<div className="w-1/3 flex justify-between space-x-4">
										{navigation.map((item) => (
											<a key={item.name} href={item.href} className="transition ease-in-out delay-150 text-black px-3 py-2 rounded-lg hover:bg-black hover:text-white" aria-current={item.current ? "page" : undefined}>
												{item.name}
											</a>
										))}
									</div>
							...
							<div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
								<div className="flex-shrink-0">
									<button className="bg-violet-800 px-10 py-2 drop-shadow-2xl rounded-lg text-white hover:text-white hover:animate-pulse transition ease-in-out duration-200 hover:scale-105">Contact Me</button>
								</div>
							</div>
						</div>
					</div>
					...
					<Disclosure.Panel className="sm:hidden">
						<div className="space-y-1 px-2 pt-2 pb-3">
							{navigation.map((item) => (
								<Disclosure.Button key={item.name} as="a" href={item.href} className="block px-5 py-5 rounded-md text-base font-medium" aria-current={item.current ? "page" : undefined}>
									{item.name}
								</Disclosure.Button>
							))}
						</div>
